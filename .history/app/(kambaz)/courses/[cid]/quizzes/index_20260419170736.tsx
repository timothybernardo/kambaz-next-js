/**
 * Quiz List screen for a course.
 * Fetches all quizzes on mount, filters by search text and publish status
 * (students only see published quizzes), and renders each as a row with
 * title, availability, due date, points, and question count.
 * Handles the + Quiz button, publish toggle, delete confirmation, and
 * the Edit/Delete/Publish dropdown menu for faculty.
 */
...
export default function Quizzes() {
  ...
  const visibleQuizzes = (isFaculty ? courseQuizzes : courseQuizzes.filter(q => q.published))
              ...
              visibleQuizzes.map(quiz => {
                ...
                return (
                  <ListGroupItem key={quiz._id}
                    ...
                    <FaRocket className="me-3 fs-3 text-success" />
                    ...
                    <div className="flex-grow-1" style={{ cursor: "pointer" }}
                      ...
                      <div style={{ fontSize: "14px" }}>
                        ...
                        {getAvailability(quiz).startsWith("Not available until") && (
                          <span>
                            <strong className="text-dark">Not available until </strong>
                            <span>
                              {getAvailability(quiz).replace("Not available until ", "")}
                            </span>
                          </span>
                        )}
                        {due && <> | <strong>Due</strong> <span>{due}</span></>}
                        {" "}| {quiz.points} pts
