    ...
    for (const q of questions) {
      ...
      if (q.type === "Multiple Choice") {
        ...
        const match = q.possibleAnswers?.some((a: any) => {
          ...
          const isCorrect = typeof a === "string" ? true : a.isCorrect;
          return isCorrect && text.toLowerCase() === String(answer ?? "").toLowerCase();
        });
  ...
  return (
    <div id="wd-quiz-preview" style={{ maxWidth: 680, margin: "0 auto", padding: "20px" }}>
      ...
      {visibleQuestions.map((q, idx) => {
        ...
        return (
          <div key={q._id} className="d-flex align-items-start mb-3">
            {/* Flag icon */}
            <div className="me-2 mt-1" style={{ cursor: "pointer" }}
              onClick={() => setFlagged(f => ({ ...f, [globalIdx]: !f[globalIdx] }))}>
              <span style={{
                fontSize: 22,
                color: flagged[globalIdx] ? "orange" : "#ccc"
              }}>⚑</span>
            </div>
      ...
      <div className="d-flex justify-content-end align-items-center border rounded p-3 mb-3">
        {!submitted ? (
          <>
            <span className="text-muted me-3" style={{ fontSize: 14 }}>Not saved</span>
