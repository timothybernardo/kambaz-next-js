...
function QuestionEditor({ question, onChange, onDelete }:
  ...
  const handleTypeChange = (type: QuestionType) => {
    ...
    if (type === "Multiple Choice")
      ...
      onChange({ ...base, possibleAnswers: [{ text: "", isCorrect: true }], choices: undefined, correctAnswer: undefined });
  };
          ...
          <div>
            ...
            {question.possibleAnswers?.map((ans, ai) => (
              <div key={ai} className="d-flex align-items-center gap-2 mb-3">
                <span style={{ color: ans.isCorrect ? "green" : "transparent", fontSize: 20 }}>➤</span>
                <span className="text-muted" style={{ width: 120, textAlign: "right", cursor: "pointer" }}
                  onClick={() => onChange({
                    ...question,
                    possibleAnswers: question.possibleAnswers!.map((a, i) => ({ ...a, isCorrect: i === ai }))
                  })}>
                  {ans.isCorrect
                    ? <span className="text-success fw-bold">Correct Answer</span>
                    : "Possible Answer"}
                </span>
                <FormControl style={{ width: 220 }} value={ans.text}
                  onChange={e => onChange({
                    ...question,
                    possibleAnswers: question.possibleAnswers!.map((a, i) =>
                      i === ai ? { ...a, text: e.target.value } : a)
                  })} />
                <FaTrash className="text-secondary ms-2" style={{ cursor: "pointer" }}
            ...
            <div className="text-end">
              <span className="text-danger" style={{ cursor: "pointer" }}
                onClick={() => onChange({
                  ...question,
                  possibleAnswers: [...(question.possibleAnswers ?? []), { text: "", isCorrect: false }]
                })}>
                + Add Another Answer
              </span>
            </div>
